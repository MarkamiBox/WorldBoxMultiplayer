...
namespace WorldBoxMultiplayer
{
    public class NetworkManager : MonoBehaviour
    {
        ...
        public void StartHost(int port) { try { _server = new TcpListener(IPAddress.Any, port); _server.Start(); _server.BeginAcceptTcpClient(OnClientConnected, null); _isHost = true; IsConnected = true; IsMapLoaded = true; WorldBoxMultiplayer.instance.UpdateStatus("Waiting for players..."); StartCoroutine(SyncCheckerRoutine()); Debug.Log("Server Started."); } catch (Exception e) { Debug.LogError("Host Error: " + e.Message); } }
        public void StartClient(string ip, int port) { try { _client = new TcpClient(); _client.NoDelay = true; _client.Connect(ip, port); _stream = _client.GetStream(); IsConnected = true; IsMapLoaded = false; WorldBoxMultiplayer.instance.UpdateStatus("Connected! Waiting for Map..."); StartCoroutine(SyncCheckerRoutine()); Debug.Log("Client Connected!"); } catch (Exception e) { WorldBoxMultiplayer.instance.UpdateStatus("Connection Failed"); Debug.LogError("Client Error: " + e.Message); } }

        private void OnClientConnected(IAsyncResult ar) { try { _client = _server.EndAcceptTcpClient(ar); _client.NoDelay = true; _stream = _client.GetStream(); Debug.Log("Client joined!"); _shouldStartTransfer = true; } catch (Exception e) { Debug.LogError("Conn Error: " + e.Message); } }

        void Update() {
            if (_shouldStartTransfer) { WorldBoxMultiplayer.instance.UpdateStatus("Sending Map..."); SaveTransferHandler.Instance.StartTransfer(); _shouldStartTransfer = false; }
            ...
            if (_stream.DataAvailable) {
                try {
                    ...
                    if (bytesRead > 0) {
                        ...
                        _incomingBuffer.Clear(); _incomingBuffer.Append(content);
                    }
        ...
        private void ProcessPacket(string packet) {
            try {
                string[] parts = packet.Split('|'); if (parts.Length < 1) return; string type = parts[0];
        ...
        public void Disconnect() { try { SendRaw("D\n"); if (_stream != null) _stream.Close(); if (_client != null) _client.Close(); if (_server != null) _server.Stop(); } catch {} IsConnected = false; _stream = null; _client = null; _server = null; _isHost = false; IsMapLoaded = false; _incomingBuffer.Clear(); WorldBoxMultiplayer.instance.UpdateStatus("Disconnected"); }
